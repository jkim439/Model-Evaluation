...
from dicom.pixel_data_handlers.util import get_expected_length, pixel_dtype
...
SUPPORTED_TRANSFER_SYNTAXES = [
    dicom.UID.JPEGBaseline,
    dicom.UID.JPEGExtended,
    dicom.UID.JPEGLosslessP14,
    dicom.UID.JPEGLossless,
    dicom.UID.JPEGLSLossless,
    dicom.UID.JPEGLSLossy,
    dicom.UID.JPEG2000Lossless,
    dicom.UID.JPEG2000,
]
...
def create_data_element(dicom_dataset):
    ...
    if dicom_dataset.file_meta.TransferSyntaxUID.is_compressed:
        if getattr(dicom_dataset, 'NumberOfFrames', 1) > 1:
            pixel_data_sequence = dicom.encaps.decode_data_sequence(
                dicom_dataset.PixelData)
        else:
            pixel_data_sequence = [
                dicom.encaps.defragment_data(dicom_dataset.PixelData)
            ]
